    ...
    manager.connect(id, conn, size || { cols: 100, rows: 30 }, {
      ...
      onStatus: (s) => {
        ...
        if (s.state === 'connected' || s.state === 'error') {
          setTimeout(() => resolve({ get output() { return output; }, statuses }), 300);
        }
      },
      onClose: () => {},
    });
...
(async () => {
  ...
  const rejecting = new SshManager(
    new KnownHosts(path.join(tmpDir, 'kh-reject.json')),
    async () => false
  );
  ...
  check('người dùng từ chối host key thì kết nối bị chặn',
    rejected.statuses.some((s) => s.state === 'error'), rejected.statuses);
  ...
  const manager = new SshManager(new KnownHosts(khPath), async (info) => {
    askedCount += 1;
    check('vân tay host key đúng định dạng SHA256', /^SHA256:[A-Za-z0-9+/]{43}$/.test(info.fingerprint), info.fingerprint);
    return true;
  });
  ...
  check('đăng nhập bằng mật khẩu thành công', s1.statuses.some((s) => s.state === 'connected'), s1.statuses);
  ...
  check('vân tay được ghi vào known_hosts', fs.existsSync(khPath) && !!JSON.parse(fs.readFileSync(khPath, 'utf8'))['127.0.0.1:' + port]);
  const s2 = await connectSession(manager, 's2', {
    host: '127.0.0.1', port, username: USERNAME, authType: 'password', password: PASSWORD,
  });
  check('lần sau không hỏi lại host key nữa', askedCount === 1, { askedCount });
  check('phiên thứ hai chạy song song được', s2.statuses.some((s) => s.state === 'connected'));

  // --- 4. Kích thước pty truyền đúng ---
  check('pty được cấp đúng cols/rows và term',
    ptyLog[0] && ptyLog[0].cols === 100 && ptyLog[0].rows === 30 && ptyLog[0].term === 'xterm-256color',
    ptyLog[0]);

  manager.resize('s1', 132, 43);
  await wait(300);
  check('đổi kích thước cửa sổ được gửi sang server',
    ptyLog.some((p) => p.resized && p.cols === 132 && p.rows === 43), ptyLog.filter(p => p.resized));
  ...
  const bad = await connectSession(manager, 's-bad', {
    host: '127.0.0.1', port, username: USERNAME, authType: 'password', password: 'sai-be-bet',
  });
  check('sai mật khẩu thì báo lỗi chứ không treo',
    bad.statuses.some((s) => s.state === 'error'), bad.statuses);

  // --- 8. Đăng nhập bằng private key ---
  const s4 = await connectSession(manager, 's4', {
    host: '127.0.0.1', port, username: USERNAME, authType: 'key', privateKeyPath: clientKeyPath,
  });
  ...
  check('thiếu mật khẩu đã lưu thì báo lỗi rõ ràng',
    noKey.statuses.some((s) => s.state === 'error' && s.message.includes('chưa lưu mật khẩu')), noKey.statuses);

  const missingKey = await connectSession(manager, 's-missingkey', {
    host: '127.0.0.1', port, username: USERNAME, authType: 'key',
    privateKeyPath: path.join(tmpDir, 'khong-ton-tai'),
  });
  check('private key không tồn tại thì báo lỗi rõ ràng',
